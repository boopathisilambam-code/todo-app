...
function Todo() {
  ...
  const deleteTodo = async (id) => {
    try {
      await axios.delete(`${import.meta.env.VITE_API_URL}/api/todos/${id}`, getAuthHeaders());
  ...
  const toggleComplete = async (id) => {
    ...
    try {
      const res = await axios.put(`${import.meta.env.VITE_API_URL}/api/todos/${id}`, { completed: !currentTodo.completed }, getAuthHeaders());
  ...
  const saveEdit = async (id) => {
    ...
    try {
      const res = await axios.put(`${import.meta.env.VITE_API_URL}/api/todos/${id}`, { text: editText }, getAuthHeaders());
      setTodolist(todolist.map((item) => (item._id === id ? res.data : item)));
      setEditingId(null);
    } catch (err) {
      showError('Failed to update task.');
